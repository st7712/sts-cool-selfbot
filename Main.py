...
{Fore.CYAN}! Logged in as: {Fore.WHITE} {Xenon.user.name}#{Xenon.user.discriminator} {Fore.CYAN} ID: {Fore.WHITE}{Xenon.user.id}   
...
async def on_connect():
    ...
    startprint()
    ctypes.windll.kernel32.SetConsoleTitleW(f'[Cool st Selfbot v{SELFBOT.__version__}] | Logged in as {Xenon.user.name}')
...
@Xenon.command(aliases=['cmds', 'commands'])
async def help(ctx):
    await ctx.message.delete()
    print(f'''
...
{Fore.BLUE}crashvid {Fore.LIGHTBLACK_EX}- Sends The Crash Video (If Someone Clicks On The Video, It Will Crash Their Discord Client)
...
async def backup(ctx):
    await ctx.message.delete()
    print(f'''{Fore.BLUE}[LOG] {Fore.WHITE}Command ran [Backup]'''+Fore.RESET)
    for user in Xenon.user.friends:
        try:
            with open('Backup\Friends.txt', 'a+') as f:
...
@Xenon.command(aliases=["stopstream", "stopplay", "stoplisten"])
async def stopactivity(ctx):
    print(f'''{Fore.BLUE}[LOG] {Fore.WHITE}Command ran [Stopactivity]'''+Fore.RESET)
    await ctx.message.delete()
    await Xenon.change_presence(activity=None, status=discord.Status.dnd)
